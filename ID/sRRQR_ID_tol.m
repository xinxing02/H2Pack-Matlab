function [R, p] = sRRQR_ID_tol(A, f, tol)

%
%   Strong Rank Revealing QR with an error threshold tol
%   (SPECIAL VERSION, ONLY FOR ID)
%       A(:, p) = Q * R = Q [R11, R12; 
%                              0, R22]
...
%   Furthermore, each column of R22 has norm less than 'tol'.
%   
%   Input: 
...
%
%   Note: 
%       Algorithm 5 in the above ref. is implemented.
%
...
%   partial pivoting QR
[R, p, k] = QRpartial(A, 'tol', tol);

%   change diagonals of R to be positive.
if (size(R,1) == 1 || size(R,2) == 1)
    ss = sign(R(1,1));
else
    ss = sign(diag(R));
end
ss = [ss; ones(size(R,1)-length(ss),1)];
R = bsxfun(@times, R, reshape(ss, [], 1));

%   special case: each column has norm less than the threshold. 
if (k == 0)   
    R = zeros(0, size(A, 2));
    p = 1 : size(A, 2);
    return ;
...
%   special case: r == n, full column rank, no need for any operation
if (k == n)   
    R = R(1:k, :);
    return ;
end
...
while 1
    ...
    %%   STEP 2: Check whether k -> k-1 can meet the threshold.
    [tmp, i] = min(omega);    
    %   rank (k-1) doesn't meet the threshold, stop
    if tmp > tol
        break 
    end

    %   interchanging the i and k th columns
    if i < k
        p(i:k)     =  p([(i+1):k, i]);
        R(:, i:k)  =  R(:, [(i+1):k, i]);
        %   givens rotation for the triangulation of R(1:k, 1:k)
    ...
    %   Update A^{-1}B, omega, gamma (can be improved)
    AB = linsolve(R(1:k, 1:k), R(1:k, (k+1):end), struct('UT', true)); % A^{-1} * B
    gamma = (sum(R((k+1):end, (k+1):end).^2, 1).^(1/2))';
    tmp = linsolve(R(1:k, 1:k), eye(k), struct('UT', true));
    omega = sum(tmp.^2, 2).^(-1/2);    
end
